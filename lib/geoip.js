...
class GeoIP {
	constructor (options) {
		...
		this.dbUri = 'https://download.maxmind.com/app/geoip_download?edition_id='+this.options.source+'&license_key='+this.options.geoIPKey+'&suffix=tar.gz';
		...
		var updatePromise = tmp.file({ prefix: 'geoip-', postfix: '.mmdb' }).then((tmpFile) => {
			...
			return new Promise((resolve, reject) => {
				maxmind.open(tmpFile.path, {
					cache: {
						max: this.options.maxmindCache // max items in cache
					}
				}, (err, lookup) => {
					if (err) {
						reject(err);
